 ...
 * 由 UI 层在启动时通过 `LaunchProfileBuilder.setMenuBackgroundProvider` 注入。
 * 这样 core 模块不依赖 video 模块，避免循环依赖。
 *
 * 若未注入实现（provider == null），启动器行为与之前完全一致，不影响任何功能。
 *
 * Android 版本：从 Java 接口移植为 Kotlin interface。
 */
interface MenuBackgroundProvider {

    /**
     * 在指定 gameDir 中安装主菜单背景资源包。
     *
     * 实现应完成：
     * 1. 检查缓存（按视频文件路径 + size + mtime 哈希命名），命中则复用
     * 2. 缓存未命中：用 JavaCV 从视频按时间间隔提取 6 帧，缩放裁剪为正方形 PNG
     * 3. 打包为资源包 zip（含 pack.mcmeta + 6 张 panorama_*.png）
     * 4. 复制/写入到 `gameDir/resourcepacks/<返回值>`
     *
     * 调用方（LaunchProfileBuilder）负责随后调用 [OptionsTxtWriter.enableResourcePack]
     ...
     */
    fun installTo(gameDir: Path, videoPath: Path, cacheDir: Path, mcVersion: String): String?
}
